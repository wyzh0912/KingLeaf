...
class CEval:
    ...
    def run(self, shot: int, split: str):
        ...
        for task_name in self.TASK2DESC:
            ...
            task_dir= task_name+ "_val.csv"
            result, acc = self.run_single_task(task_name, task_dir,shot, split)
    ...
    def run_single_task(self, task_name: str, task_dir: str,shot: int, split: str):
        File_Dir="ceval/ceval-exam"
        dataset=pd.read_csv(os.path.join(File_Dir,"val",task_dir))
        results = []
        acc = 0
        for index, data in tqdm(dataset.iterrows()):
            prompt = f"以下是中国关于{self.TASK2DESC[task_name]}考试的单项选择题，请给出正确答案的选项。\n"
            if shot != 0:
            ...
            response = self.tokenizer.decode(outputs[0, inputs.size(1):]).strip()
            answer = response[0]
            results.append(
                {
                    "prompt": prompt,
...
def main():
    ...
    tokenizer = AutoTokenizer.from_pretrained(
        '/d2/model/ckpt_sft', model_max_length=2048
    )
    model = model.eval()
